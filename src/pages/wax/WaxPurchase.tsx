...
export const WaxPurchase: React.FC = () => {
  const [waxDetails, setWaxDetails] = useState<WaxDetails | null>(null);
  const [showAddCardModal, setShowAddCardModal] = useState(false);
  const [showBuyWaxModal, setShowBuyWaxModal] = useState(false);
  const [showAutoReloadModal, setShowAutoReloadModal] = useState(false);

  const loadWaxDetails = useCallback(() => {
    waxService.getDetails().then(setWaxDetails);
  }, []);
  ...
  const hasPaymentMethod = !!waxDetails.userStripeSource;
      ...
      <div className="w-full">
        {showAddCardModal && (
          <AddCardModal
            ...
            onSuccess={loadWaxDetails}
          />
        )}
        {showBuyWaxModal && (
          <BuyWaxModal
            open={true}
            onClose={() => setShowBuyWaxModal(false)}
            onSuccess={loadWaxDetails}
          />
        )}
        {showAutoReloadModal && (
          <WaxAutoReloadSettingModal
            isOpen={true}
            onClose={() => setShowAutoReloadModal(false)}
            initialData={waxDetails.autoReloadSettings}
            onSuccess={loadWaxDetails}
          />
        )}

        <div className="w-full flex justify-center min-h-full mt-10">
          <div className="w-full max-w-[1280px] flex flex-col items-center justify-start gap-4 p-5">
            <Container>
              <div className="flex flex-col gap-4 md:gap-2 md:flex-row md:items-start md:justify-between mb-6">
                ...
                {hasPaymentMethod && (
                  <Button
                    onClick={() => setShowBuyWaxModal(true)}
                    variant="outline"
                    size="md"
                  >
                    Add Wax
              ...
              <div className="w-full grid grid-cols-1 md:grid-cols-[300px_1fr] gap-6">
                <div className="flex flex-col items-center justify-center text-center h-[200px] bg-[#16202A] rounded-lg border border-[#304962] p-4 transition-colors">
                  <h2 className="text-[32px] font-bold text-[#F98B8D] mb-1">
                    {typeof waxDetails.remainingCredits === 'number'
                      ? waxDetails.remainingCredits.toLocaleString()
                      : waxDetails.remainingCredits}
                  </h2>
                    ...
                    {!hasPaymentMethod ? (
                      <div className="flex flex-col items-start gap-4">
                        <span className="text-[#F3F4F6] text-sm">
                          Please add a payment method to make any purchase.
                        </span>
                        <Button
                      ...
                      <>
                        ...
                        <div className="flex items-center gap-4 mb-5">
                          <div className="rounded-md border border-[#304962] px-3 py-2.5">
                            <CardDetail card={waxDetails.userStripeSource} title="" />
                          </div>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-4">
                          <div className="flex items-start gap-3">
                            <div className="text-sm text-[#F3F4F6]">
                              {waxDetails.autoReloadSettings.enabled ? (
                                <div className="flex items-center gap-1">
                                  ...
                                  <span className="font-bold">
                                    ${waxDetails.autoReloadSettings.amount}
                                  </span>{' '}
                                  worth of wax when your wax balance drops below{' '}
                                  <span className="font-bold">
                                    {waxDetails.autoReloadSettings.threshold?.toLocaleString()}
                                    .
                                  </span>
                                </div>
                              ) : (
                          ...
                          <Button
                            onClick={() => setShowAutoReloadModal(true)}
                            variant="outline"
                            size="md"
                          >
                            Edit Settings
